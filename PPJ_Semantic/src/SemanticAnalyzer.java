...
public class SemanticAnalyzer {
	...
	private void unarni_izraz(Node node) throws SemanticAnalyserException {
		...
		if (context.isProduction("<unarni_izraz> ::= <postfiks_izraz>")) {
			...
			check(context.child2);
			if (!(implicitInt(context.child2.getSymbolInfo().dataType.get(0))
					&& context.child2.getSymbolInfo().l_expr == true)) {
				throw new SemanticAnalyserException("<unarni_izraz> ::= OP_INC <unarni_izraz>");
			}
			context.symbolInfo.dataType.add(DataType.INT);
			context.symbolInfo.l_expr = false;
		} else if (context.isProduction("<unarni_izraz> ::= OP_DEC <unarni_izraz>")) {
			check(context.child2);
			if (!(implicitInt(context.child2.getSymbolInfo().dataType.get(0))
					&& context.child2.getSymbolInfo().l_expr == true)) {
				throw new SemanticAnalyserException("<unarni_izraz> ::= OP_DEC <unarni_izraz>");
			}
			context.symbolInfo.dataType.add(DataType.INT);
			context.symbolInfo.l_expr = false;
		} else {
			check(context.child2);
			if (!implicitInt(context.child2.getSymbolInfo().dataType.get(0))) {
				throw new SemanticAnalyserException("<unarni_izraz> ::= <unarni_operator> <cast_izraz>");
			}
			context.symbolInfo.dataType.add(DataType.INT);
			context.symbolInfo.l_expr = false;
		}
