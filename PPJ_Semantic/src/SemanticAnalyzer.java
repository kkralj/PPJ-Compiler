...
public class SemanticAnalyzer {
    ...
    private void deklaracija_parametra(Node node) throws SemanticAnalyserException {
        ...
        if (context.isProduction("<deklaracija_parametra> ::= <ime_tipa> IDN")) {
            ...
            context.symbolInfo.addAllData(context.firstChild.getSymbolInfo().dataType);
    ...
    private void izravni_deklarator(Node node) throws SemanticAnalyserException {
        ...
        if (context.isProduction("<izravni_deklarator> ::= IDN")) {
            ...
            if (inheritableType.peek().equals(DataType.VOID) || scope.isDeclaredLocally(name)) {
                System.out.println(inheritableType.peek());
                System.out.println(scope.isDeclaredLocally(name));
                throw new SemanticAnalyserException(node);
            }
    ...
    private void inicijalizator(Node node) throws SemanticAnalyserException {
        ...
        if (context.isProduction("<inicijalizator> ::= <izraz_pridruzivanja>")) {
            ...
            context.symbolInfo.addAllData(node.getChild(1).getSymbolInfo().dataType);
        }
    ...
    private void lista_parametara(Node node) throws SemanticAnalyserException {
        ...
        if (context.isProduction("<lista_parametara> ::= <deklaracija_parametra>")) {
            ...
            context.symbolInfo.addAllArguments(context.firstChild.getSymbolInfo().argumentNames);
            context.symbolInfo.addAllData(context.firstChild.getSymbolInfo().dataType);
            ...
            context.symbolInfo.addAllData(paramListSymbolInfo.dataType);
            // only one data type here
            context.symbolInfo.addAllData(declParamSymbolInfo.dataType);

            context.symbolInfo.addAllArguments(paramListSymbolInfo.argumentNames);
            // only one name here
            context.symbolInfo.addAllArguments(declParamSymbolInfo.argumentNames);
        }
    }
    ...
    private void primarni_izraz(Node node) throws SemanticAnalyserException {
        ...
        if (context.isProduction("<primarni_izraz> ::= IDN")) {
            ...
            if (scope.isDeclared(name)) {
                context.symbolInfo.addAllData(scope.getSymbolInfo(name).dataType);
